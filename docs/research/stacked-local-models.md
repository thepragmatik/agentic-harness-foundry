...
For Foundry, the default architecture is therefore:
...
A 2025 reassessment of Mixture-of-Agents found that repeated sampling from one high-quality model (`Self-MoA`) often outperformed mixtures of different models. The lesson for a laptop harness is important: **diversity only helps when the added models contribute sufficiently high-quality complementary outputs**.
...
| Model | Strength relevant to Foundry | Why it is not the primary resident candidate today | Possible future specialist role |
|---|---|---|---|
| `openbmb/MiniCPM5-2B` | Current on-device model; official GGUF/llama.cpp path; long-context and tool-calling positioning; Apache-2.0 | Lower capability ceiling than the 9B distill is likely for evidence-preserving compression/code analysis; adding it beside a 9B model creates switching complexity | Ultra-cheap extraction/classification or draft model **only if** a measured high-volume job justifies it; DSpark is separately interesting after baseline selection |
| `Qwen/Qwen3.5-2B` | Same Qwen family; Apache-2.0; modern compact multimodal architecture | Using 2B and 9B Qwen variants concurrently has limited architectural diversity; a separate 2B service must beat the 9B on end-to-end latency/energy enough to justify switching | Fast narrow schema/extraction path if benchmarked from production telemetry |
| `LiquidAI/LFM2.5-2.6B` | Explicitly designed for on-device/agentic workloads; hybrid conv+GQA architecture can be efficient | Custom `lfm1.0` license and another architecture/runtime path; no current evidence that it improves our few chosen utility tasks enough to justify a second resident model | Efficiency challenger if the primary model proves too slow; not part of initial qualification |
| `HuggingFaceTB/SmolLM3-3B` | Fully open Apache-2.0, compact, GGUF ecosystem, reasoning/tool-use positioning | Older 2025 generation and no obvious unique job versus newer compact models; would widen decision surface without a concrete gap | Simple extraction/summarization fallback only if already-local startup cost is materially lower |
| `microsoft/Phi-4-mini-instruct` | MIT license, strong compact reasoning/code pedigree | Different prompt/tool conventions and older generation; no concrete Foundry workload currently requires it over the primary candidate | Difficult small-model reasoning challenger if a narrow gap is observed |
| `ibm-granite/granite-4.2-3b` | Very recent, Apache-2.0, native tool calling, low-effort/full thinking modes; trained on agentic traces including Hermes | The 3B model does **not** receive the full SWE/terminal/search agentic-RL block that IBM applies to 8B/30B; still likely lower utility ceiling than the 9B primary | Most credible tiny specialist if we later need a second model for tool-planning/structured agentic work |
...
**Conditionally recommended.** A 2–3B model handles a single very frequent cheap task, the 9B resident handles harder local utility tasks, and cloud remains the final escalation path. Promote only if the specialist reduces end-to-end latency/energy or frees the resident model enough to matter.

### Pattern C — several local models vote/fuse every answer

**Not recommended.** It multiplies prompt prefill and decode work, adds orchestration and memory pressure, and has no obvious cost advantage for this laptop deployment. Use only for a deliberately scoped offline evaluation.

### Pattern D — different models per agent role

**Not initially recommended.** Planner/model-A + executor/model-B + verifier/model-C sounds attractive but compounds per-step error and context transfer. Prefer deterministic tools/verifiers for execution and validation; split model roles only after telemetry identifies a stable specialization.

## The one-small-model rule
...
5. The specialist improves **end-to-end accepted-task latency, energy, or external-token savings** materially.
6. The architecture remains operable if the specialist is disabled.

Until then, the original models remain a **candidate shelf**, not an active model pool.
